...
class FlappyGraph:
	def __init__(self, img_size):
		...
		W1 = tf.Variable(tf.truncated_normal([img_size, L1], stddev=0.01, dtype=tf.float32))
		b1 = tf.Variable(tf.zeros(L1))

		W2 = tf.Variable(tf.truncated_normal([L1, output_dim], stddev=0.01, dtype=tf.float32))
		b2 = tf.Variable(tf.zeros(output_dim))

		y1 = tf.nn.relu(tf.matmul(self.inputs, W1) + b1)
		self.y_logits = tf.matmul(y1, W2) + b2

		self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.y_logits, labels=self.actions))
