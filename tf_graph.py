...
class FlappyGraph:
    def __init__(self, img_size):
        ...
        b3 = tf.Variable(tf.ones(output_dim) * -2.8)
        ...
        self.new_prob = ((self.actions - 1) + self.sigmoid) * (2 * self.actions - 1)
        self.loss = tf.reduce_mean(self.rewards * tf.log(self.new_prob + 1e-7))
